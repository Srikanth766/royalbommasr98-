...
API_ID = int(environ.get('API_ID', '20892645')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', 'f4ac7191e0544afa519161b4bcae3865')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "8246920694:AAH9bVuzuc7JurW7Egho9RrU_c7WjaLh-7I")    # Bot token from @BotFather
...
PICS = (environ.get('PICS', 'https://graph.org/file/88bea6b7ff736bd140628-8b47a59fd9613501c6.jpg https://graph.org/file/5f0b8bee882cf83cb1e1f-2db87f8f451a3bc2eb.jpg https://graph.org/file/425b6f46efc7c6d64105f.jpg https://graph.org/file/82a285998dab2a9c1d658-977a455addcfd7e701.jpg')).split()  # Sample pic
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1824418433 8031488277').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002654190322').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1003182001493'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1003182001493'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1003182001493'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1003149277481').split()] #(make sure bot is admin)
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1004827634734')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1003186511908')  # Request channel id (make sure bot is admin)
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/+nle8UoKle2Q3YmQ1')  # Support group link (make sure bot is admin)

# FORCE_SUB 
auth_req_channels = environ.get("AUTH_REQ_CHANNELS", "-1003195810206")# requst to join Channel for force sub (make sure bot is admin) only for bot ADMINS  
auth_channels     = environ.get("AUTH_CHANNELS", "-1003195810206")# Channels for force sub (make sure bot is admin)

# ============================
# Payment Configuration
# ============================
QR_CODE = environ.get('QR_CODE', 'https://i.ibb.co/nsn0b4jx/image.jpg')    # QR code image for payments
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://srikanthreddycmd77:KzkIWsMnrv0tDxS3@cluster0.hquo2bt.mongodb.net/?appName=Cluster0")  # MongoDB URI for the database
DATABASE_NAME = environ.get('DATABASE_NAME', "Cluster0") # Database name (default: cluster)
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'telugumovies_files') # Collection name (default: dreamcinezone_files)
...
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1003186511908'))  # Notification of sent to your channel
...
LOG_VR_CHANNEL = int(environ.get('LOG_VR_CHANNEL', '-1003240303662')) #Verification Channel Id 
LOG_API_CHANNEL = int(environ.get('LOG_API_CHANNEL', '-1003240303662')) #If Anyone Set Your Bot In Any Group And Set Shortner In That Group Then In This Channel The All Details Come
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/+xko9GYv3zQ4xOTA1') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/@pspk3519') # Owner link for the bot
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', 'https://t.me/+Ahn08Hkta_UwZTBl') # Update channel link for the bot

# ============================
# User Configuration
# ============================
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '6224341518').split()]
